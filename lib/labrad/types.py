...
def evalLRData(s):
    ...
    def _DimensionlessArray(x): # created by Youpeng.
        return U.DimensionlessArray(x)

    def _ValueArray(x,unit):
        if unit is None:
            unit = ''
        return U.ValueArray(x, unit)

    globs = globals()
    
    globs['DimensionlessArray'] = _DimensionlessArray
    globs['ValueArray'] = _ValueArray
